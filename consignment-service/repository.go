...
// MarshalConsignment an input vessel type to a vessel model
...
func (mr *MongoRepository) Create(ctx context.Context, consignment *Consignment) error {
	_, err := mr.collection.InsertOne(ctx, consignment)
	return err
}

func (mr *MongoRepository) GetAll(ctx context.Context) ([]*Consignment, error) {
	cur, err := mr.collection.Find(ctx, nil, nil)
